...
def report_grades( student, gradebk ):
    ...
    and create .txt file for e-mail
    message to student with grades.
    ...
    # Get grades message for student
    grade_msg = range(len(filter(lambda x: 'HW' in x, student_data.columns.values)))
    for i in xrange(len(grade_msg)):
        score = student_data['HW' + str(i + 1)].iloc[0]
        grade_msg[i] = 'For problem set #' + str(i + 1) + ', you earned a score of: ' + score + '.'
    
    ########### Create e-mail message for student ###########
    
    # Standard messages
    greeting = 'Hello, History of Mathematics Student!'
    gen_info = 'The following is a report of the grades currently recorded for your problem sets. Please report any errors to Professor ***** or your TA *****. Thank you!'
